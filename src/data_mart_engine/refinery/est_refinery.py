...
class EstRefinery:
    ...
    def refine_estimator_data(self):
        ...
        # 1. Standardize Timestamp (The Gold Bridge)
        # We ensure this is a Series of strings so DuckDB treats it as VARCHAR
        if 'wall_ns' in df.columns:
            ...
            # Use current time as an anchor for all 78 rows
        ...
        refined_df = pd.DataFrame({
            ...
            'mission_id': df['mission_id'].astype(str) if 'mission_id' in df.columns else "default_mission",
        ...
        # Your existing save_gold_fact handles the overwrite/write logic safely
