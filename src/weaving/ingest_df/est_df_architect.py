...
class EstDFArchitect:
    """EST domain ingestion into parquet shards"""
    def __init__(self, bin_path, limit=5000, vault_b=None):
        ...
        self.buffer = []
        # EST whitelist
        self.whitelist = ["XKF1", "NKF1", "XKF2", "NKF2", "IMU"]
        ...
        while True:
            ...
            if msg.get_type() in self.whitelist:
                ...
                if captured_count % 2000 == 0:
                    print(f"  🔹 Processed {raw_count} raw / {captured_count} EST messages...")

        self.flush()
        print(f"\n🛡️ EST INTEGRITY REPORT")
        print(f"Total Raw Packets Read : {raw_count}")
        print(f"EST Packets Captured   : {captured_count}")
