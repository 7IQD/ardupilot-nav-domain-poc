class NavActionMap:
    ...
    def evaluate(cls, stats):
        ...
        avg_hdop = stats.get('avg_hdop', 99.0)

        if stats.get('sample_count', 0) < 10:
            return {"verdict": "UNKNOWN", "score": 0, "status": "INCOMPLETE", "message": "⚠️ Insufficient data."}
        ...
        msg = f"P95 Drift: {p95:.2f}m | HDOP: {avg_hdop:.2f}"
        if verdict == "FAIL": msg += " — 🚨 Safety violation! Check EKF tuning."

        return {"verdict": verdict, "score": score, "status": status, "message": msg}